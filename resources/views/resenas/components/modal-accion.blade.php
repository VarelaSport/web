    ...
    <div id="modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96 relative">
            ...
            <form action="/resenas/{{$resena->id}}/actualizar" method="post" class="d-inline">
                @csrf
                @method('put')
                <h2 class="text-lg font-semibold mb-4 text-center">¿Aprobar o rechazar la reseña?</h2>
                <p class="mb-4 text-gray-700 text-center">{{$resena->comentario}}</p>
                <p class="hidden">{{$resena->id}}</p>
